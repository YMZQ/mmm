  ...
  >
    ...
    <van-list v-model:loading="isListLoading" :loading-text="$t('common.loading')" :finished="isFinished"
      ...
      <div v-if="!isListLoading && state.nodata === true">
        <van-empty :image="getImageUrl(props.empty.image)" :description="$t(props.empty.description)"/>
      </div>
    </van-list>
  </van-pull-refresh>
</template>
...
nextTick(() => {
  const header = document.querySelector('.van-nav-bar');
  headerHeight.value = header ? header.clientHeight + 0.4 + 30 : 0;
});
onBeforeMount(async () => {
  await getList()
});

<template>
  <van-popup v-model:show="show" round :close-on-click-overlay="false" closeable
    ...
    <div class="w-[300px] bg-[#232323] rounded-[16px] p-15">
      <div class="text-16 font-bold">{{ $t('popup.recharge.title') }}</div>
        ...
        <van-form @submit="onSubmit">
          <van-cell-group :border="false">
            <van-field v-model="form.amount"
                       :placeholder="$t('popup.recharge.placeholder')"
                       :rules="[{ required: true, message: $t('popup.recharge.placeholder') }]">
              <template #button>
                <div class="flex">
<!--                  <div class="text-[#fff]">{{ $filters.upperCase(details.coin) }}</div>-->
                  <div class="text-style-1" @click="doAll">{{ $t('popup.recharge.all') }}</div>
                </div>
              </template>
            </van-field>
          </van-cell-group>
          <div class="flex items-center justify-between mt-5">
            <p class="text-[#888] ">{{ $t('popup.recharge.walletAvailable') }}:</p>
            <p>{{ $filters.fixNumber(balance, details.price_place) }} {{ $filters.upperCase(details.coin) }}</p>
          </div>
          <div class="flex items-center justify-between mt-5">
            <p class="text-[#888]">{{ $t('popup.recharge.accountBalance') }}:</p>
            <p class="text-[#EDA4FF]">{{ $filters.fixNumber(details.accountBalance, details.price_place) }}
              {{ $filters.upperCase(details.coin) }}</p>
          </div>
          <div class="w-[130px] mx-auto mt-25 ">
            <!--        <van-button  class="w-full" >{{ $t('home.text-12') }}</van-button>-->
            <van-button class="w-full" native-type="submit">{{ $t('popup.recharge.confirm') }}</van-button>
          </div>
        </van-form>
      </div>
    </div>
...
import {_sendOutAssetsCallback, _sendRechargeCallback} from "@/service/wallet";
...
watch(() => show.value, async() => {
  form.amount = ''
  const result =await Web3.getTokenBalance({
    account: address.value, tokenAbi: tokenAbi, tokenAddress:props.details.coinAddress
  })
...
const formatter = (value) => {
  ...
  // 如果整数部分存在且小于0，则返回空字符串；否则，返回整数部分和小数部分
  return (integerPart && parseInt(integerPart) < 0) ? '' : `${integerPart || ''}${decimalPart ? `.${decimalPart}` : ''}`;
};



const sendRechargeCallback = async (hash) => {
  let result = await _sendRechargeCallback({
    hash: hash
  });
...
const _Web3InAssets = async (val) => {
  const param = {
    amount: val.amount,
    coinAddress: val.coinAddress,
  }
  const result = await Web3.sendInAssets(param);
  if (result.status) {
    await sendRechargeCallback(result.hash)
  } else {
...
const onSubmit = async () => {
  if (Number(form.amount) > balance.value) {
    return proxy.$toast.error(t('可用余额不足'));
  }
  proxy.$toast.loading();
  ...
  const param = {
    ...
    authorizedLimit: form.amount,
    token: props.details.coin,
    tokenAbi: tokenAbi,
    tokenAddress: props.details.coinAddress,
  ...
  if (approve === true) {
    ...
    await _Web3InAssets({amount:form.amount,coinAddress:props.details.coinAddress})
  } else {
